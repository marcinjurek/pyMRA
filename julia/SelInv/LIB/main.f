...
C       ****************************************************************
C
C       MAIN.F :  DRIVER THAT SOLVES A SPARSE, SYMMETRIC, NONSINGULAR
C       LINEAR SYSTEM VIA SPARSE LDL' FACTORIZATION.  
...
C       (6) IWMAX   -   UPPER BOUND ON INTEGER WORK STORAGE.
C
...
C       --------------------------------------------------------------
     ...
     &                      TMPMAX
        PARAMETER       (   NMAX        = 18240,
     &                      ANZMAX      = 616480,
     &                      SUBMAX      = MAX(ANZMAX,616480),
     &                      LNZMAX      = 2737840,
     &                      TMPMAX      = 351560,
     &                      IWMAX       = 7*NMAX + 3    )
C
...
C       COUNTS FOR L (SFINIT OR ORDERING ROUTINE).
C       ----------------------------------------------
...
C       THEN IT CONTAINS THE ENTRIES OF THE LDL' FACTOR (BLKFCT).
C       -----------------------------------------------------------------
        DOUBLE PRECISION    LNZ(LNZMAX)
        DOUBLE PRECISION    DIAG(NMAX)
...
C       ------------------------------
        ...
        INTEGER             CACHSZ, I     , ICASE , IFLAG , LEVEL ,
     &                      MATUNT, OUTUNT
...
C
C           INPUT  : N
C           OUTPUT : INVP, PERM, SFIFLG
C           --------------------------------------------------------
            TIMBEG = GTIMER()
            CALL  ORDNAT  (  N     , PERM  , INVP  , SFIFLG )
            TIMEND = GTIMER()
            TIME = TIMEND - TIMBEG
            WRITE (OUTUNT,*) ' '
            WRITE (OUTUNT,77) 
...
C
C           INPUT  : N, XLINDX, LINDX, IWMAX
C           OUTPUT : PERM, INVP, NSUB, IFLAG, NNZL, COLCNT, NSUPER,
C                    XSUPER, SNODE, SFIFLG
C           WORK   : IWORK(4*N)
C           -------------------------------------------------------
            TIMBEG = GTIMER()
            CALL  ORDMMD (  OUTUNT, N     , XLINDX, LINDX , INVP  , 
     &                      PERM  , IWMAX , IWORK , NNZL  , NSUB  , 
     &                      COLCNT, NSUPER, XSUPER, SNODE , SFIFLG,
     &                      IFLAG                                   )
            TIMEND = GTIMER()
            TIME = TIMEND - TIMBEG
            IF  ( IFLAG .NE. 0 )  THEN
                WRITE (OUTUNT,*) '***** ERROR CODE ',IFLAG,
     &                           ' RETURNED FROM ORDMMD *****'
                GO TO 1700
            ENDIF
            WRITE (OUTUNT,77) 
     &          '   TIME FOR ORDERING                  = ', TIME
...
C
C           INPUT  : N, NNZA, XADJ, ADJ, PERM, INVP, IWMAX
C           OUTPUT : PERM, INVP, COLCNT, NNZL, NSUB, NSUPER, SNODE, XSUPER,
C                    IFLAG
C           WORK   : IWORK(7*N+3) [MAXIMUM REQUIRED BY ANY ROUTINE.]
C           ---------------------------------------------------------------
            TIMBEG = GTIMER()
            CALL  SFINIT  ( OUTUNT, N     , NNZA  , XADJ  , ADJ   , 
     &                      PERM  , INVP  , COLCNT, NNZL  , NSUB  , 
     &                      NSUPER, SNODE , XSUPER, IWMAX , IWORK , 
     &                      IFLAG                                   )
            TIMEND = GTIMER()
            TIME = TIMEND - TIMBEG
            IF  ( IFLAG .NE. 0 )  THEN
                WRITE (OUTUNT,*) '***** ERROR CODE ',IFLAG,
     &                           ' RETURNED FROM SFINIT *****'
                GO TO 1700
            ENDIF
            WRITE (OUTUNT,*) ' '
            WRITE (OUTUNT,77) 
...
C                XSUPER, XLINDX, LINDX, XLNZ, IWMAX
C       OUTPUT : LNZ, IFLAG
C       WORK   : IWORK(N)
C       ----------------------------------------------------
        TIMBEG = GTIMER()
        CALL  INPNV  (  OUTUNT, N     , XADJD , ADJD  , ANZ   , 
     &                  PERM  , INVP  , NSUPER, XSUPER, XLINDX, 
     &                  LINDX , XLNZ  , LNZ   , IWMAX , IWORK , 
     &                  IFLAG                                   )
        TIMEND = GTIMER()
        TIME = TIMEND - TIMBEG
        IF  ( IFLAG .NE. 0 )  THEN
...
C
C       -----------------------------------------------------------
C       INSUFFICIENT FLOATING POINT WORK STORAGE FOR FACTORIZATION?
C       -----------------------------------------------------------
...
C                LNZ, IWMAX, TMPSIZ, MMPY[LVL], SMXPY[LVL]
C       OUTPUT : LNZ, DIAG, IFLAG
C       WORK   : IWORK(2*N+2*NSUPER), TMPVEC(TMPSIZ)
C       -----------------------------------------------------------
        TIMBEG = GTIMER()
        IF  ( LEVEL .EQ. 1 )  THEN
            CALL  BLKFCT (  OUTUNT, N     , NSUPER, XSUPER, SNODE , 
     &                      SPLIT , XLINDX, LINDX , XLNZ  , LNZ   , 
     &                      DIAG  , IWMAX , IWORK , TMPSIZ, TMPVEC, 
     &                      IFLAG , MMPY1 , SMXPY1                  )
...
C
C       ------------------------------------------------------
C       BLKSLV : NUMERICAL SOLUTION USING LDL' FACTORIZATION.
C
